...
Place your trained touch-detection AI model files here.

## Structure

Each AI model plugin lives in its own sub-directory:

```
ai_model_plugins/
└── my_model/
    ├── my_model_plugin.py     ← AI model plugin adapter file
    └── my_weights.pth         ← Trained PyTorch weights
```

## How to create an AI model plugin

1. Create a sub-directory under `ai_model_plugins/`.
...
4. Restart the detector app: the AI model plugin is automatically discovered and shown in the model dropdown.
