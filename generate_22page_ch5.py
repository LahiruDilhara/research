...
def get_chapter5_tex():
    return r"""\chapter{Implementation and Software Design}
...
\hline
\textbf{Input:} Raw video file path $S_{\text{video}}$, Source Frame Rate $R_{\text{src}}$ (30/60 FPS), Target Frame Rate $R_{\text{target}} = 12\text{ FPS}$ \\
\textbf{Output:} Downsampled video frame sequence matrix $\mathbf{V}_{\text{down}}$ \\
1: Open video stream \texttt{cap = cv2.VideoCapture(S\_video)}. \\
2: Calculate frame step sampling ratio: $k_{\text{step}} = \text{round}(R_{\text{src}} / R_{\text{target}})$. \\
3: Initialize frame index $n = 0$, sampled buffer array $\mathbf{V}_{\text{down}} = []$. \\
4: \textbf{while} \texttt{cap.isOpened()} \textbf{do} \\
5: \quad Read frame $\text{ret}, \mathbf{I}_n = \text{cap.read}()$. \\
6: \quad \textbf{if} $\text{ret}$ is False \textbf{then} Break loop. \\
7: \quad \textbf{if} $n \pmod{k_{\text{step}}} == 0$ \textbf{then} \\
8: \qquad Compute Laplacian blur metric: $b = \text{cv2.Laplacian}(\mathbf{I}_n, \text{cv2.CV\_64F}).\text{var}()$. \\
9: \qquad \textbf{if} Blur score $b \ge \theta_{\text{blur}}$ ($100.0$) \textbf{then} \\
10: \qquad \quad Append frame $\mathbf{I}_n$ to sequence array $\mathbf{V}_{\text{down}}$. \\
11: \qquad \textbf{end if} \\
12: \quad \textbf{end if} \\
13: \quad Increment frame index $n = n + 1$. \\
14: \textbf{end while} \\
15: \textbf{return} Frame sequence matrix $\mathbf{V}_{\text{down}}$ \\
\hline
\end{tabular}
...
\hline
\textbf{Input:} Calibration checkerboard images $\{\mathbf{I}_m\}_{m=1}^K$, Grid dimensions $(W_{\text{board}}, H_{\text{board}})$, Square size $s_{\text{square}} = 25.0\text{ mm}$ \\
\textbf{Output:} Intrinsic matrix $K \in \mathbb{R}^{3 \times 3}$, Lens distortion vector $\mathbf{D} \in \mathbb{R}^5$ \\
1: Define 3D object grid points $\mathbf{P}_{\text{obj}} = \{(i \cdot s, j \cdot s, 0)\}_{i=0, j=0}^{W_{\text{board}}-1, H_{\text{board}}-1}$. \\
2: Initialize empty object point list $\mathcal{O}_{\text{pts}}$ and image point list $\mathcal{I}_{\text{pts}}$. \\
3: \textbf{for} each calibration image $\mathbf{I}_m$ \textbf{do} \\
4: \quad Convert $\mathbf{I}_m$ to grayscale $\mathbf{I}_{\text{gray}}$. \\
5: \quad Find checkerboard corners: $\text{found}, \mathbf{C}_{\text{raw}} = \text{cv2.findChessboardCorners}(\mathbf{I}_{\text{gray}}, (W_{\text{board}}, H_{\text{board}}))$. \\
6: \quad \textbf{if} $\text{found}$ is True \textbf{then} \\
7: \qquad Refine corners: $\mathbf{C}_{\text{sub}} = \text{cv2.cornerSubPix}(\mathbf{I}_{\text{gray}}, \mathbf{C}_{\text{raw}}, \text{winSize}=(11,11))$. \\
8: \qquad Append $\mathbf{P}_{\text{obj}}$ to $\mathcal{O}_{\text{pts}}$ and $\mathbf{C}_{\text{sub}}$ to $\mathcal{I}_{\text{pts}}$. \\
9: \quad \textbf{end if} \\
10: \textbf{end for} \\
11: Execute Tsai pinhole camera calibration: $\text{RMS}, K, \mathbf{D}, \mathbf{R}_{\text{vecs}}, \mathbf{T}_{\text{vecs}} = \text{cv2.calibrateCamera}(\mathcal{O}_{\text{pts}}, \mathcal{I}_{\text{pts}})$. \\
12: \textbf{return} Intrinsic matrix $K$, Lens distortion vector $\mathbf{D}$ \\
\hline
\end{tabular}
...
with open('chapters/chapter05.tex', 'w') as f:
    f.write(get_chapter5_tex())

print("Wrote expanded chapter05.tex")
