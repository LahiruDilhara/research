...
# Instead of one fixed smoothing value, alpha now ADAPTS to how many
# markers are visible: few markers -> trust the new solve LESS (higher
# alpha, lean on history) since it's poorly conditioned and noisy.
# Many markers -> trust the new solve MORE (lower alpha, respond fast)
# since it's well conditioned and the noise mostly cancels out.
H_SMOOTHING_ALPHA_MIN = 0.35   # used when marker count is high (>= ALPHA_FULL_CONFIDENCE_MARKERS)
H_SMOOTHING_ALPHA_MAX = 0.92   # used when marker count is at MIN_MARKERS_REQUIRED
ALPHA_FULL_CONFIDENCE_MARKERS = 5  # marker count at/above which we use the MIN alpha
...
def refine_corners_subpixel(gray, corners):
    """
    Refine AprilTag corner detections to sub-pixel accuracy using
    cv2.cornerSubPix. This reduces the pixel-level jitter AT THE SOURCE,
    which is the root cause of homography flicker — especially valuable
    when few markers are visible, since there's less averaging to hide
    that noise later.
    """
    corners_f32 = corners.reshape(-1, 1, 2).astype(np.float32)
    criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.01)
    refined = cv2.cornerSubPix(
        gray, corners_f32, winSize=(5, 5), zeroZone=(-1, -1), criteria=criteria
    )
    return refined.reshape(-1, 2)


def compute_adaptive_alpha(num_markers_used):
    """
    Map the number of visible/known markers to a smoothing strength.
    Fewer markers -> higher alpha (lean on history, since the fresh
    solve is noisier and less trustworthy).
    More markers -> lower alpha (respond faster, since the fresh solve
    is well-conditioned and trustworthy).
    """
    if num_markers_used >= ALPHA_FULL_CONFIDENCE_MARKERS:
        return H_SMOOTHING_ALPHA_MIN
    if num_markers_used <= MIN_MARKERS_REQUIRED:
        return H_SMOOTHING_ALPHA_MAX
    # linear interpolation between the two ends
    span = ALPHA_FULL_CONFIDENCE_MARKERS - MIN_MARKERS_REQUIRED
    frac = (num_markers_used - MIN_MARKERS_REQUIRED) / span
    return H_SMOOTHING_ALPHA_MAX + frac * (H_SMOOTHING_ALPHA_MIN - H_SMOOTHING_ALPHA_MAX)
    ...
    while True:
        ...
        for tag in tags:
            ...
            if tag.tag_id in TAG_LAYOUT_MM:
                ...
                # Sub-pixel refine the raw corners before using them —
                # this reduces the pixel-noise that causes flicker,
                # especially important when few markers are visible.
                refined_corners = refine_corners_subpixel(gray, tag.corners)

                image_pts.extend(refined_corners.tolist())
        ...
        if num_markers_used >= MIN_MARKERS_REQUIRED:
            ...
            if H_new is not None:
                if smoothed_H is None:
                    ...
                    alpha = compute_adaptive_alpha(num_markers_used)
                    smoothed_H = alpha * smoothed_H + (1 - alpha) * H_new
