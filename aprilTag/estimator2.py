...
def circular_mean_deg(angles_deg):
    """
    Average a list of angles (in degrees) correctly, accounting for
    wraparound (e.g. averaging -179 and 179 should give ~180, not 0).
    A plain arithmetic mean breaks near the +/-180 boundary; this
    converts each angle to a unit vector, averages the vectors, and
    converts back.
    """
...
def main():
    ...
    while True:
        ...
        # Collect per-tag pose values so we can average them across
        # every marker visible in this frame.
        distances_m = []
        rolls_deg = []
        pitches_deg = []
        yaws_deg = []
        ...
        # --- Average pose across all markers currently visible ---
        # Distance: plain mean is fine (no wraparound issue).
        # Angles: use circular mean so e.g. tags reading -179 and 179
        # deg don't average to 0.
        if len(tags) > 0:
            avg_distance_m = float(np.mean(distances_m))
            avg_roll = circular_mean_deg(rolls_deg)
            avg_pitch = circular_mean_deg(pitches_deg)
            avg_yaw = circular_mean_deg(yaws_deg)

            summary_lines = [
                f"Markers visible: {len(tags)}",
                f"Avg dist: {avg_distance_m*100:.1f} cm",
                f"Avg roll:  {avg_roll:6.1f} deg",
                f"Avg pitch: {avg_pitch:6.1f} deg",
                f"Avg yaw:   {avg_yaw:6.1f} deg",
            ]
        else:
            summary_lines = ["Markers visible: 0"]

        # Draw the summary panel in the top-left corner of the frame
        panel_x, panel_y = 10, 25
        for i, line in enumerate(summary_lines):
            cv2.putText(
                frame, line, (panel_x, panel_y + i * 22),
