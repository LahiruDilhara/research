...
\midrule
\textbf{1:} & Extract wrist origin coordinates $\mathbf{P}_0(t) = (x_0(t), y_0(t))$. \\
\textbf{2:} & Extract middle MCP landmark coordinates $\mathbf{P}_9(t) = (x_9(t), y_9(t))$. \\
\textbf{3:} & Compute unitless hand length scale factor: $L_{\text{hand}}(t) = \sqrt{(x_9(t) - x_0(t))^2 + (y_9(t) - y_0(t))^2}$. \\
...
\textbf{9:} & \hspace{1em} Calculate normalized position: $x_i^{\text{norm}}(t) = \frac{x_i(t) - x_0(t)}{L_{\text{hand}}(t)}$, \hspace{1em} $y_i^{\text{norm}}(t) = \frac{y_i(t) - y_0(t)}{L_{\text{hand}}(t)}$. \\
\textbf{10:} & \hspace{1em} Calculate numerical velocity: $v_{x,i}(t) = \frac{x_i^{\text{norm}}(t) - x_i^{\text{norm}}(t-1)}{\Delta t}$, \hspace{1em} $v_{y,i}(t) = \frac{y_i^{\text{norm}}(t) - y_i^{\text{norm}}(t-1)}{\Delta t}$. \\
...
\textbf{13:} & Direct raw feature propagation (preserve contact deceleration transients). \\
...
\midrule
\textbf{1:} & Append current feature vector $\mathbf{f}_t$ to sliding queue $\mathbf{Q}_{\text{win}}$. \\
...
\textbf{5:} & Construct sliding window input matrix $\mathbf{X}_W = [\mathbf{f}_{t-4}, \mathbf{f}_{t-3}, \mathbf{f}_{t-2}, \mathbf{f}_{t-1}, \mathbf{f}_t]^T \in \mathbb{R}^{5 \times 84}$. \\
\textbf{6:} & Advance sliding queue by stride of 3 frames (retaining 2-frame overlap). \\
...
\textbf{8:} & Execute PyTorch model forward pass in evaluation mode (\texttt{with torch.no\_grad():}): \\
...
\textbf{12:} & \hspace{2em} \hspace{1em} Inject system virtual key press event via OS API (e.g., `'A'`). \\
...
\midrule
\textbf{1:} & Map viewport pixel coordinate to scene millimeter space: $[x_{\text{scene}}, y_{\text{scene}}, 1]^T = \mathbf{T}_{\text{view}}^{-1} \cdot [u_{\text{m}}, v_{\text{m}}, 1]^T$. \\
\textbf{2:} & Compute grid-snapped origin coordinates: \\
...
\textbf{5:} & Define bounding box: $x_{\min} = x_{\text{snap}}$, $y_{\min} = y_{\text{snap}}$, $x_{\max} = x_{\text{snap}} + W_{\text{key}}$, $y_{\max} = y_{\text{snap}} + H_{\text{key}}$. \\
\textbf{6:} & Check bounding box collision against existing keys: $\text{Overlap} = \text{CheckIntersections}(B_{\text{new}}, \{B_{\text{existing}}\})$. \\
\textbf{7:} & \textbf{if} Overlap is True \textbf{then} \\
\textbf{8:} & \hspace{1em} Shift origin $x_{\text{snap}} = x_{\text{snap}} + \Delta g$ and recompute $B_{\text{new}}$. \\
\textbf{9:} & \textbf{end if} \\
\textbf{10:} & Instantiate \texttt{KeyButtonGraphic} graphics item on \texttt{QGraphicsScene}. \\
...
\textbf{3:} & \hspace{1em} Read raw video frame: $\text{ret}, \mathbf{I}_{\text{raw}} = \text{cap.read}()$. \\
...
\textbf{7:} & \hspace{2em} Discard oldest frame (Drop frame to prevent buffer bloat and lag). \\
\textbf{8:} & \hspace{2em} $\mathbf{Q}_{\text{frame}}.\text{get\_nowait}()$. \\
\textbf{9:} & \hspace{1em} \textbf{end if} \\
\textbf{10:} & \hspace{1em} Push new frame $\mathbf{I}_{\text{raw}}$ to $\mathbf{Q}_{\text{frame}}$. \\
...
\midrule
\textbf{1:} & Open video stream \texttt{cap = cv2.VideoCapture(S\_video)}. \\
\textbf{2:} & Calculate frame step sampling ratio: $k_{\text{step}} = \text{round}(R_{\text{src}} / R_{\text{target}})$. \\
...
\textbf{8:} & \hspace{2em} Compute Laplacian blur metric: $b = \text{cv2.Laplacian}(\mathbf{I}_n, \text{cv2.CV\_64F}).\text{var}()$. \\
\textbf{9:} & \hspace{2em} \textbf{if} Blur score $b \ge \theta_{\text{blur}}$ ($100.0$) \textbf{then} \\
\textbf{10:} & \hspace{2em} \hspace{1em} Append frame $\mathbf{I}_n$ to sequence array $\mathbf{V}_{\text{down}}$. \\
...
\textbf{15:} & \textbf{return} Frame sequence matrix $\mathbf{V}_{\text{down}}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\textbf{1:} & Define 3D object grid points $\mathbf{P}_{\text{obj}} = \{(i \cdot s, j \cdot s, 0)\}_{i=0, j=0}^{W_{\text{board}}-1, H_{\text{board}}-1}$. \\
...
\textbf{4:} & \hspace{1em} Convert $\mathbf{I}_m$ to grayscale $\mathbf{I}_{\text{gray}}$. \\
\textbf{5:} & \hspace{1em} Find checkerboard corners: $\text{found}, \mathbf{C}_{\text{raw}} = \text{cv2.findChessboardCorners}(\mathbf{I}_{\text{gray}}, (W_{\text{board}}, H_{\text{board}}))$. \\
\textbf{6:} & \hspace{1em} \textbf{if} $\text{found}$ is True \textbf{then} \\
\textbf{7:} & \hspace{2em} Refine corners: $\mathbf{C}_{\text{sub}} = \text{cv2.cornerSubPix}(\mathbf{I}_{\text{gray}}, \mathbf{C}_{\text{raw}}, \text{winSize}=(11,11))$. \\
...
The benchmark engine tested five neural network families:
\begin{enumerate}
    ...
    \item \textbf{1D Residual Networks (1D ResNet):} Deep 1D convolutional residual networks with skip connections preventing vanishing gradients.
    \item \textbf{Self-Attention and Transformer Networks (Attention):} Multi-head self-attention mechanisms modeling pairwise temporal relationships across window frames.
\end{enumerate}

Each architecture was evaluated across multiple feature input representations: raw pixel coordinates, wrist-centered coordinates, scale-normalized positions ($\mathbf{P}^{\text{norm}} \in \mathbb{R}^{42}$), numerical velocities ($\mathbf{V} \in \mathbb{R}^{42}$), and concatenated scale-normalized position plus velocity vectors ($\mathbf{f}_t = [\mathbf{P}^{\text{norm}}, \mathbf{V}] \in \mathbb{R}^{84}$). Across 5 temporal frames, this constructs the complete $(5 \times 84)$ window input tensor $\mathbf{X}_W$, providing both spatial joint geometry and dynamic impact deceleration signatures.
...
\begin{itemize}
    ...
    \begin{enumerate}
        \item \emph{Keystroke Action:} Assigns key IDs to single keys (such as `'A'`, `'Space'`, `'Enter'`).
