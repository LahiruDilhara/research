...
To ensure methodological transparency and auditability, the literature retrieval and selection process followed the Preferred Reporting Items for Systematic Reviews and Meta-Analyses (PRISMA 2020) principles \cite{ReviewVirtualKeyboard2020}:
\begin{enumerate}
    \item \textbf{Databases and Information Sources:} Comprehensive searches were conducted across IEEE Xplore, ACM Digital Library, Scopus, ScienceDirect, and Google Scholar, covering peer-reviewed articles published between 2000 and 2025.
    \item \textbf{Boolean Search Syntax:} Systematic keyword strings combined domain concepts: \texttt{("virtual keyboard" OR "paper keyboard" OR "optical keyboard") AND ("computer vision" OR "monocular RGB" OR "MediaPipe" OR "hand tracking") AND ("touch detection" OR "homography" OR "fiducial tracking")}.
    \item \textbf{Inclusion Criteria:} Peer-reviewed papers presenting empirical virtual keyboard systems, touch contact algorithms on planar surfaces, fiducial marker tracking under perspective tilt, and real-time hand pose estimation.
    \item \textbf{Exclusion Criteria:} Studies requiring wearable data gloves or intrusive physical markers, purely conceptual or non-peer-reviewed white papers, and systems requiring specialized multi-camera motion capture rigs.
    \item \textbf{Synthesis Approach:} A thematic and critical synthesis model was applied, comparing sensing modalities, multi-finger support, computational latency, and environmental robustness to derive the foundational research gaps.
\end{enumerate}
