...
Current virtual keyboard solutions face significant practical barriers. They either depend on expensive specialized hardware (such as 3D depth cameras, infrared curtains, or high-end GPUs) or suffer from severe usability flaws (such as single-finger limitations, artificial dwell-time delays, and rigid layouts). As a result, users with standard computers and everyday webcams cannot benefit from flexible, paper-based virtual input.

\subsection{General Problem}
\label{subsec:general_problem}
Physical keyboards are expensive to replace, vulnerable to liquid spills and dust, and prone to pathogen transmission in public and clinical settings \cite{ReviewVirtualKeyboard2020}. Although vision-based virtual keyboards offer a flexible alternative, existing implementations rely on specialized hardware not found in ordinary desktop environments \cite{Lee2019Virtual, Pilkov2024Markerless}, or require heavy GPU resources to process high-resolution video streams in real time.

\subsection{Specific Problem}
\label{subsec:specific_problem}
Developing an accurate, real-time, and customizable paper virtual keyboard using a single monocular RGB camera and a standard commodity CPU presents six key technical challenges:
\begin{enumerate}
    \item \textbf{Single-Finger Tracking Restrictions:} Prior monocular camera systems track only a single active finger, typically the index digit \cite{Thomas2013Camera, Ji2018Local, Srivastava2012RealTime}. Shadow-based techniques fail when multiple fingers move together because their shadows overlap and occlude one another \cite{Posner2012Single}.
    \item \textbf{Touch Ambiguity and Artificial Dwell Delays:} Without depth sensors, distinguishing between a surface touch and a finger hovering a few millimeters above the paper is difficult \cite{Thomas2013Camera, Posner2012Single}. To resolve this ambiguity, previous systems forced users to freeze their finger over a key for 500 to 1000 ms, severely limiting typing speed \cite{Khare2019QWERTY, Chen2024Gaze}.
    \item \textbf{Computational Overhead on Standard CPUs:} Modern deep learning architectures often require GPU acceleration. Running hand tracking and deep sequence models concurrently on a standard CPU can cause frame drops and high input latency \cite{Enkhbat2020HandKey, Ji2018Local}.
    \item \textbf{Sensitivity to Lighting, Shadows, and Skin Tones:} Skin-color thresholding and shadow tracking fail under diffuse room lighting, multiple illumination sources, ambient shadows, and diverse skin tones \cite{Thomas2013Camera, Yue2014Blind}.
    \item \textbf{Perspective Distortion and Paper Movement:} Camera perspective tilt and paper shifts distort key positions, leading to key registration errors without real-time homography tracking \cite{Wang2016AprilTag2, Kallwies2020Determining, Kalaitzakis2021Fiducial}.
    \item \textbf{Rigid Layouts and Lack of Action Multiplexing:} Existing virtual keyboards use fixed, hard-coded layouts \cite{ReviewVirtualKeyboard2020}. Users cannot design custom arrangements or assign different action sets to the same physical sheet.
\end{enumerate}
