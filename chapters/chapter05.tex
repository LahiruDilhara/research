...
\begin{enumerate}
    ...
    \item \textbf{Sub-Phase 6 (5-Frame Sliding Window \& PyTorch LSTM Inference):} The feature engineering module buffers 84-dimensional feature vectors $\mathbf{f}_t$ across 5 consecutive frames into temporal matrix $\mathbf{X}_W \in \mathbb{R}^{5 \times 84}$. PyTorch model \texttt{best\allowbreak\_finger\allowbreak\_touch\allowbreak\_lstm.pth} evaluates $\mathbf{X}_W$ in under $2.1\text{ ms}$, outputting per-finger touch probabilities $\mathbf{P}_{\text{touch}} \in [0, 1]^5$. If probability $p_k > 0.90$, digit $k$ is classified as contacting the paper surface.
...
{\small 13: \textbf{CRITICAL OVERRIDE check:} Confirm zero 1€ low-pass filtering applied.} \\
