...
arch_cnn1d.py
=============
Architecture 4: 1D CNN on combined coordinates + velocities.
Input shape: (N, 5, 16)  — permuted to (N, 16, 5) for Conv1d.

10 hyperparameter configurations vary conv_channels, fc_hidden, dropout, lr, batch_size.
"""
...
from model_arch import TouchCNN1D, normalize, make_loaders, train_config, evaluate_model, save_result
...
CONFIGS = [
    {"id": 1, "conv_ch": 64, "fc_hid": 64, "dropout": 0.20, "lr": 1e-3, "bs": 32},
]
...
def main():
    ...
    for cfg in CONFIGS:
        print(f"\n  [Config {cfg['id']:02d}/10]  conv_ch={cfg['conv_ch']}  fc_hid={cfg['fc_hid']}  "
