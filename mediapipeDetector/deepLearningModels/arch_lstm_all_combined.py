...
CONFIGS   = [{'id': 1, 'hidden': 48, 'layers': 2, 'dropout': 0.25, 'lr': 0.001, 'bs': 32}]

def create_model(feature_dim, cfg):
    return SequenceLSTM(input_features=feature_dim, hidden_units=cfg.get("hidden", 48), num_layers=cfg.get("layers", 2), dropout=cfg.get("dropout", 0.25))
