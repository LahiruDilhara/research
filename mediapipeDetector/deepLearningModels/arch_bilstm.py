...
arch_bilstm.py
==============
Architecture 7: Bidirectional LSTM on combined coordinates + velocities.
Input shape: (N, 5, 16) — processed in both temporal directions.

BiLSTM doubles the effective hidden state by concatenating forward & backward outputs.
...
from model_arch import (
    TouchBiLSTM, normalize, make_loaders,
...
RESULTS_CSV = Path(__file__).resolve().parent / "results" / "bilstm.csv"
...
# hidden: per-direction hidden units  (effective output = hidden * 2)
CONFIGS = [
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=len(CONFIGS)) as executor:
        ...
        for future in as_completed(futures):
            ...
            with _print_lock:
                print(
                    f"  ✓ cfg{cfg['id']:02d}  hidden={cfg['hidden']:3d}×2  layers={cfg['layers']}  "
    ...
    for r in results:
        ...
        print(f"  cfg{cfg['id']:02d}  Best: {r['best_acc']:6.2f}%  "
              f"F1: {r['rpt']['Touch']['f1-score']:.4f}  "
              f"hidden={cfg['hidden']}×2  layers={cfg['layers']}  lr={cfg['lr']}")
    print(f"  {'─'*60}")
