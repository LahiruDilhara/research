...
from annotator.constants import (
    ...
    MISSING_FRAMES_TOLERANCE, MODEL_PATH,
)
...
def process_video(
    ...
    while True:
        ret, frame = cap.read()
        if not ret:
            if header_total > 0 and fi < header_total:
                logger.warning(f"Video reader reached EOF early at frame {fi}/{header_total}")
            break
    ...
    logger.info(f"MediaPipe video processing finished. Extracted {actual_total} frames, duration {duration_ms} ms.")
